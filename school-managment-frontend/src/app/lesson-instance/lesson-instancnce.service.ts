...
export class LessonInstanceService {
    ...
    private url: string = environment.apiUrl + 'lessonsInstances';
    ...
    private getLessonInstances(teacherUrl, date) {
        const promise = new Promise<LessonInstance[]>((resolve) => {
            const lessonInstancesCall = this.http.get(this.url + '/search/findByDateAndTeacher' +
                '?date=' + date +
                '&&teacher=' + teacherUrl).subscribe((data: { _embedded }) =>
